...
export function TeamScheduleDialog({ tasks, users, onCreated }: { tasks: TaskOpt[]; users: UserOpt[]; onCreated?: () => void }) {
  const [open, setOpen] = useState(false)
  const [taskId, setTaskId] = useState('')
  const [userId, setUserId] = useState('')
  const [mode, setMode] = useState<'recurring' | 'one'>('recurring')
  ...
  async function submit() {
    ...
    setSaving(true)
    const payload: any = { taskId, userId, timeOfDay, durationMin }
    if (mode === 'one') {
  ...
  return (
    <div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogHeader>Schedule Task For User</DialogHeader>
        ...
        <DialogFooter>
          ...
          <Button onClick={submit} disabled={!taskId || !userId || saving}>{saving ? 'Saving…' : 'Save'}</Button>
        </DialogFooter>
      </Dialog>
    </div>
  )
